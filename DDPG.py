...
class DDPG(AbstractSolver):
    ...
    def train_episode(self):
        ...
        state = np.ndarray.flatten(np.array(state))
        for _ in range(self.options.steps):
            ...
            self.memorize(state, act, ret[1], np.ndarray.flatten(np.array(ret[0])), ret[2])

            self.replay()

            self.update_target_networks()

            state = np.ndarray.flatten(np.array(ret[0]))
    ...
    def saveModel(self):
        torch.save(self.actor_critic, 'actorCritic.pth')
        torch.save(self.target_actor_critic, 'targetActorCritic.pth')
        torch.save(self.optimizer_q, "optimizerQ.pth")
        torch.save(self.optimizer_pi, "optimizerPi.pth")

        import csv

        # field names
        fields = ['State (Joint/Mug QPos)', 'Action']
        ...
        with open(filename, 'w+') as csvfile:
            ...
            for i in range(0, len(self.acts)):
                csvwriter.writerows(zip(self.states, self.acts))
