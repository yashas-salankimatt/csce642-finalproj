...
import Available_solvers as avs
...
def getEnv(domain, render_mode=""):
    try:
        if options.disable_plots:
            return gym.make(domain, render_mode='human')

        return gym.make(domain, render_mode='null')
    except:
